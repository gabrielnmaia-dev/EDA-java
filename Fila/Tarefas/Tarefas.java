...
private static void mostrarFila(PriorityQueue<Tarefa> fila) {
    if (fila.isEmpty()) {
        System.out.println("   (fila vazia)");
        return;
    }

    // Criar copia para nao modificar a original
    PriorityQueue<Tarefa> temp = new PriorityQueue<>(fila);
    int pos = 1;
    while (!temp.isEmpty()) {
        System.out.println("   " + pos + "°: " + temp.poll());
        pos++;
    }
}
